      ...
      <div className='bg-white rounded-lg border-8 border-white-500 shadow-2xl w-2/3 mx-auto my-8 h-2/3 grid grid-cols-2'>
        ...
        <div className='bg-gray-100 w-full'>
          <img
            ...
            alt={name}
            className='w-full h-full xl:w-3/3 xl:h-3/3 object-contain'
          />
        </div>

        {/* Côté droit (Titre, Type, Stats) */}
        <div
          className='grid grid-rows-[40%,60%] gap-4 '
          style={{ boxShadow: '-4px 0 5px -5px black' }}
        >
          {/* Partie supérieure (Titre, Type et ID) */}
          <div
            className={`text-white ${getTypeCard(
              types[0].type.name
            )} relative w-full h-auto xl:max-w-[1000px]  lg:max-w-[90%] md:w-full md:max-w-[80%] sm:w-full sm:max-w-[50%] xs:max-w-[20%] pokemon-info-container`}
          >
            <p className='font-bold text-xl mt-5 mb-5 ml-16 '>
              #{getPokemonId()}
            </p>
            <h1 className='text-8xl font-bold uppercase mb-5 ml-10 xl:text-3xl lg:text-3xl  md:text-2xl sm:text-xl'>
              {name}
            </h1>
            ...
            <div className='flex space-x-2 ml-28'>
              {types.map((type) => (
                <span
                  key={type.type.name}
                  className={`rounded-3xl p-3 shadow-2xl w-20 text-center capitalize xl:text-md xl:w-16 lg:text-sm lg:w-14 md:text-xs md:p-2 md:w-12 sm:text-xs sm:w-10 sm:p-1  ${getTypeButton(
                    type.type.name
                  )}`}
                  style={{
                    boxShadow:
          ...
          <div>
            <div className='flex flex-wrap justify-start mt-[-16px] w-full'>
              <div className='flex gap-14 rounded-lg bg-gray-100 h-24 p-5 w-full relative lg:max-w-[400px] lg:mx-auto'>
                <div className='text-center'>
                  <p className='text-lg capitalize text-gray-500 ml-5 xl:text-md'>
                    Height
                  </p>
                  <p className='text-xl font-semibold ml-5 xl:text-md'>
                    {height / 10} m
                  </p>
                </div>
                <div className='border-r border-gray-300 h-full xl:text-md'></div>

                <div className='text-center'>
                  <p className='text-lg text-gray-500 capitalize mr-5 xl:text-md'>
                    Weight
                  </p>
                  <p className='text-xl font-semibold xl:text-md'>
                    {weight / 10} kg
                  </p>
                </div>
                ...
                <div className='text-gray-700'>
                  <p className='text-base xl:text-xs xl:max-h-[20%]'>
                    {description}
                  </p>
                </div>
              </div>
